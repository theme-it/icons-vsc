...
export const getThemeConfig = (section: string) => {
  return getConfig('theme-it-icons-vsc').inspect(section);
};

/** Update the config of the theme */
export const setThemeConfig = (section: string, value: any, global: boolean = false) => {
  return getConfig('theme-it-icons-vsc').update(section, value, global);
};
...
export const isThemeActivated = (global: boolean = false): boolean => {
  return global ? (getConfig().inspect('workbench.iconTheme').globalValue === 'theme-it-icon-vsc-light' || getConfig().inspect('workbench.iconTheme').globalValue === 'theme-it-icon-vsc')
      : (getConfig().inspect('workbench.iconTheme').workspaceValue === 'theme-it-icon-vsc-light' || getConfig().inspect('workbench.iconTheme').globalValue === 'theme-it-icon-vsc');
};
